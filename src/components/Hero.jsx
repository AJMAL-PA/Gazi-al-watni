        ...
        <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 w-full flex flex-col items-center text-center mt-20 md:mt-16">
                <motion.div
                    ...
                    className={`flex flex-col items-center w-full ${isRtl ? 'font-inter' : ''}`}
                    dir={dir}
                >
                    <h1 className={`text-white mb-4 md:mb-6 drop-shadow-2xl justify-center leading-tight ${isRtl ? 'text-5xl sm:text-6xl md:text-8xl font-black' : 'font-bebas italic text-6xl sm:text-7xl md:text-[5.5rem] lg:text-[7rem] xl:text-9xl'}`}>
                        {t('hero.heading')}
                    </h1>
                    <p className={`text-slate-200 mb-8 md:mb-12 max-w-3xl drop-shadow-lg leading-relaxed px-4 md:px-0 ${isRtl ? 'text-lg md:text-2xl font-semibold' : 'font-light font-inter text-sm md:text-lg lg:text-xl'}`}>
                        {t('hero.description')}
                    </p>
                </motion.div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 w-full max-w-xs sm:max-w-none px-6 sm:px-0">
                    ...
                    <a
                        ...
                        {t('hero.getQuote')} <span className="material-symbols-outlined text-xs sm:text-sm">arrow_forward</span>
                    </a>
                </div>
            </div>
        </section>
